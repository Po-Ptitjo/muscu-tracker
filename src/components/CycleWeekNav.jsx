    ...
    <div className="sticky top-0 z-40" style={{ background: 'rgba(7,9,15,0.97)', backdropFilter: 'blur(24px)' }}>
      <div className="border-b border-border-subtle">
        <div className="flex items-center gap-1 px-3 py-2 overflow-x-auto scrollbar-hide">
          {cycles.map((cycle) => {
            ...
            return (
              <button key={cycle.id} onClick={() => onCycleChange(cycle.id)}
                ...
                {cycle.phase && (
                  <span className="font-body text-[9px] tracking-widest uppercase"
                    style={{ color: isActive ? cycle.color + 'AA' : '#2A3A4A' }}>
                    {cycle.phase}
                  </span>
                )}
              </button>
          ...
          <button onClick={onGenerateCycle}
      ...
      <div className="flex items-center px-3 py-1.5 gap-1 overflow-x-auto scrollbar-hide border-b border-border-subtle">
        {weeks.map((week) => {
          ...
          return (
            <button key={week.number} onClick={() => onWeekChange(week.number)}
              ...
              <div className="flex flex-col items-start">
                ...
                {isDeload && (
                  <span className="font-body text-[9px]" style={{ color: '#F59E0B88' }}>Décharge</span>
                )}
              </div>
              <div className="flex gap-0.5">
                {week.sessions.map(s => (
                  <span key={s.id} className="block w-1.5 h-1.5 rounded-full"
                    style={{
                      background: s.status === 'done' ? '#A78BFA'
              ...
              {allDone && <span className="text-[10px]" style={{ color: '#A78BFA' }}>✓</span>}
            </button>
          )
        })}
      </div>
